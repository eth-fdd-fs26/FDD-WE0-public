"""Part 2 — MISSING: the regression head was never added.

The branches, the freeze and the fusion are all correct, but `head` is left as
an Identity, so the model hands back the 64 fused features instead of a single
temperature. The architecture probe fails at "No regression head".
"""
...
class BasinNet(nn.Module):
    def __init__(self, signal_extractor, encoder):
        ...
        self.head = nn.Identity()          # ← the regression head was never added
